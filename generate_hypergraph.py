...
def generate_hypergraph_from_file(input_file, output_path, args):
    ...
    Use columns in txt file for next-poi task:
        ...
        pseudo_session_trajectory_id, pseudo_aoi_trajectory_id, UTCTimeOffsetWeekday, UTCTimeOffsetHour.
    ...
    usecols = [
        ...
        'UTCTimeOffsetHour', 'check_ins_id', 'pseudo_session_trajectory_id', 'pseudo_aoi_trajectory_id'
    ]
    ...
    if args.view == 'session':
        ...
        traj_column = 'pseudo_aoi_trajectory_id'
    else:
        raise ValueError(f'view parameter {args.view} is not supported!')
    ...
    # todo Create sparse matrix for trajectory -> category, for cate-sharing trajectory relationship
    ...
    print(
        f'Done saving checkin2trajectory pyg data to {ci2traj_out_file}'
...
def generate_hyperedge_stat(data, traj_column):
    ...
    print(
        f'Number of hyperedges(trajectory): {traj_stat.shape[0]}\n'
        f'The min, mean, max size of hyperedges are: '
        f'{traj_stat["size"].min()}, {traj_stat["size"].mean()}, {traj_stat["size"].max()}\n'
        f'The min, mean, max time window of hyperedges are:'
...
def generate_traj2traj_data(
    ...
    print(
        f"Number of {relation_type}-user hyperedge2hyperedge(traj2traj) relation has been generated: "
        f"{traj2traj.row.shape[0]}, while threshold={threshold} and mode={filter_mode}."
    )
